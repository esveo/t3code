/**
 * Fork: one decision to answer or task to check off. The options are a choice
 * list with the coordinator's recommendation and its reason on the
 * recommended option; a task has a checkbox instead. A note, pros and cons and
 * the rarer actions stay folded until asked for.
 */
...
export function DecisionCard(props: DecisionCardProps) {
  ...
  const sourceId = decision.sourceThreadId ?? decision.coordinatorThreadId;
  const routeId =
    decision.routeToThreadId && decision.routeToThreadId !== sourceId
      ? decision.routeToThreadId
      : null;
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {props.mode === "focus" ? (
        <div className="flex flex-col gap-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {[task ? "Task" : null, URGENCY_LABELS[decision.urgency]].filter(Boolean).join(" · ")}
      ...
      <p className="text-xs text-muted-foreground">
        From{" "}
        <ThreadLinkChip
          environmentId={props.coordinatorRef.environmentId}
          threadId={sourceId}
          label={props.threadTitle(sourceId) ?? "Coordinator"}
        />
        {routeId ? (
          <>
            {" "}
            ...
            <ThreadLinkChip
              environmentId={props.coordinatorRef.environmentId}
              threadId={routeId}
              label={props.threadTitle(routeId) ?? "Thread"}
            />
          </>
        ) : null}
      </p>
      ...
      {task ? (
        <label className="flex cursor-pointer items-center gap-2.5 rounded-lg border border-input bg-accent/40 px-2.5 py-2 text-sm">
          <Checkbox
      ...
      {draft?.explain ? (
        <p className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          You ask the coordinator to explain this.
          <Button
            size="micro"
            variant="ghost-muted"
            onClick={() => props.onDraft({ explain: false })}
          >
            Undo
          </Button>
        </p>
        ...
        <Textarea
          ...
          placeholder={
            task
              ...
              : draft?.optionId
                ? "Reason or context for the coordinator"
                : "Your own answer"
          }
      ...
      <div className="flex items-center gap-0.5">
        ...
        <Button
          size="xs"
          variant="ghost-muted"
          aria-pressed={draft?.explain === true}
          onClick={() => props.onDraft({ explain: !draft?.explain })}
        >
          <LightbulbIcon />
          Explain this
        </Button>
